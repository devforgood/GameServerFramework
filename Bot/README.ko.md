...
# 서버 (별도 콘솔)
cd Game
# 실행 파일은 Game\x64\Debug 에 나온다. 리포 루트의 x64\Debug 에도 같은 이름의
# 낡은 파일이 남아 있으니 그쪽을 실행하지 말 것 — 조용히 옛날 서버가 떠서,
# 최근에 넣은 패킷이 통째로 안 오는 것처럼 보인다.
...
| `--ports <list>` | 여러 포트에 나눠 붙는다 (`65001-65016` 또는 `65001,65002`) |
| `--bots <n>` | 봇 수 |
| `--threads <n>` | 워커 스레드 수 |
...
### 서버 스레드를 늘려서 재려면 포트를 늘려야 한다

월드 하나 = 포트 하나 = 서버 스레드 하나다(`server_config.json` 의 `world.thread_count`,
그리고 그 값은 실행 인자로 연 포트 개수를 넘지 못한다). 그래서 스레드를 늘려 가며 재려면
서버도 봇도 포트를 함께 늘려야 한다. 한 포트에 몰아 붙이면 스레드를 아무리 올려도
월드 하나만 돌아서, 늘어난 스레드가 아무 일도 하지 않는다.

```powershell
# 서버: 월드 16개 + world.thread_count = 16
x64\Release\Game.exe 65001 65002 65003 65004 65005 65006 65007 65008 `
                      65009 65010 65011 65012 65013 65014 65015 65016

# 봇: 봇 번호로 16개 포트에 고르게 나눠 붙는다
.\Bot.exe --ports 65001-65016 --bots 3200 --threads 12 --duration 90 --rampup 250
```

부하가 걸린 동안 서버는 5초마다 워커별로 자기 프레임을 요약해 남긴다. "느려졌다" 가
월드 틱 때문인지 그 앞뒤 때문인지는 이 줄로만 갈린다.
...
`late` 는 한 프레임에서 10Hz 틱을 두 번 이상 따라잡아야 했던 횟수(이미 밀렸다는 뜻),
`drop` 은 따라잡기 상한을 넘겨 시뮬레이션을 버린 횟수다. 측정 결과는
`Benchmark/PERFORMANCE.md` 17절에 있다.

서버 로그가 부하의 원인이 되지 않게 `server_config.json` 의 `log.level` 을 `info` 나
`warn` 으로 낮춰 두는 것이 좋다. 기본값(`debug`)은 이동 패킷마다 한 줄을 남긴다.
...
* **선택 보상** — `봇 번호 % 선택지 수` (보상마다 완료 선택지가 하나씩 있으므로,
  봇은 그중 자기 몫을 주는 선택지를 골라 누른다)
...
**길은 하나만 보지 않는다.** 같은 노드로 가는 길이 여럿이고 각각 다른 조건이 걸려 있을 수
있다 — 분기 다음 칸이 그렇다("가지 A 를 끝냈으면"과 "가지 B 를 끝냈으면"이 같은 노드로
이어진다). 길을 하나만 골라 두면 그 길이 닫힌 봇은 다음 퀘스트를 영영 못 받는다.
그래서 목적지에 닿는 길을 전부 모아 두고 **그중 서버가 실제로 보낸 것**을 고른다.

원하는 선택지가 목록에 없으면 그것도 정보다. 대개 **레벨이 모자라** 서버가 수락 선택지를
내보내지 않은 것이므로, NPC 를 계속 두드리는 대신 잠시 물러나 몬스터를 잡는다.

포기는 **시도 횟수가 아니라 시간으로** 정한다(10분). 횟수로 세면 레벨을 올리는 데 오래 걸리는
봇이 아직 될 일을 포기해 버리고, 그러면 그 뒤 체인 전체가 선행 조건 미달로 줄줄이 막혀 그 봇은
실행 내내 아무것도 하지 않는다(45분 실플레이에서 실제로 한 봇이 이렇게 멈춰 있었다).
10분이 지나도 열리지 않으면 이번 실행에서는 받을 수 없다고 보고(`--reuse-accounts` 로 이미 끝낸
퀘스트를 다시 받으려는 경우다) 계획의 다음 칸으로 넘어간다.

건너뛴 퀘스트는 **이유와 함께 로그로 남긴다**. 조용히 건너뛰면 시나리오가 어디서 멈췄는지
알 방법이 없다 — 패킷 수치는 그대로 나오기 때문이다.

`--log debug` 로 돌리면 **목표가 바뀔 때마다** 무엇을 하려는지 남는다(상세 로그 대상 봇만).

```text
[bot_1a553_000002] 목표: 대화 quest=1002 map=1 npc=2001 gate=0 (내 맵 1, 레벨 6)
[bot_1a553_000002] 목표: 이동 quest=1002 map=2 npc=0 gate=1001 (내 맵 1, 레벨 6)
```

봇이 맵을 오가거나 제자리에 서 있을 때, 그것이 의도한 행동인지 갇힌 것인지는 이것으로만
갈린다. 실제로 이 로그가 "받을 수 있는 대화 입구가 없는 퀘스트" 를 찾아냈다 — 봇은 20초마다
NPC 를 두드리고 있었고, 겉으로는 정상적인 부하로 보였다.
...
* 상호작용 한 번이 `talk` 목표를 올리고, 완료는 대화의 완료 선택지로 접수한다.
  선택 보상이 있는 퀘스트는 완료 선택지가 보상마다 하나씩 있어 누르는 선택지가 곧 보상이다.
  **봇은 `QuestComplete` 를 직접 보내지 않는다** — 사람이 밟는 길을 그대로 밟아야
  그 길에 난 구멍을 봇이 대신 밟아 준다.
