...
```jsonc
{
  ...
  "prerequisites": {
    "completed_quest_ids": [],     // 전부 완료해야 수락 가능 (AND)
    "completed_any_quest_ids": [], // 이 중 하나만 완료하면 된다 (OR, 갈림길 합류용)
...
**합류하는 다음 칸은 `completed_any_quest_ids` 로 "둘 중 하나"를 요구한다.**
`completed_quest_ids` 는 전부 만족해야 하는 AND 라서 가지 하나를 지목하면 반대쪽을 고른
플레이어는 거기서 이야기가 끊긴다. 그렇다고 공통 조상만 요구하면 이번에는 **두 가지를
통째로 건너뛰고** 다음 칸을 받을 수 있다(대화가 막고 있을 뿐, 패킷을 직접 보내면 통과한다).

```jsonc
"prerequisites": {
  "completed_quest_ids":     [11001],          // 전부 만족해야 한다 (AND)
  "completed_any_quest_ids": [11002, 11009]    // 이 중 하나면 된다 (OR)
}
```

후보가 하나뿐이면 그냥 AND 이므로 검증이 `completed_quest_ids` 로 옮기라고 알린다.
같은 퀘스트를 양쪽에 적는 것도 막는다 — AND 가 이겨서 OR 이 뜻을 잃는다.

**대화에서 합류 지점으로 가는 길도 가지마다 하나씩 필요하다.** `show_if` 역시 퀘스트 하나만
묻기 때문에 "A 를 끝냈거나 B 를 끝냈으면"을 한 선택지로 적을 수 없다. 같은 노드로 가는
`goto` 를 조건만 바꿔 두 개 둔다(3201 이 그렇게 되어 있다). 하나만 두면 반대쪽 가지를 고른
플레이어에게는 다음 이야기로 가는 길이 아예 보이지 않는다.
...
**목표가 있는 곳까지 갈 수 있어야 한다.** 데이터 검증이 몬스터 스폰 · 드롭처 · NPC 위치 ·
`reach` 대상 맵을 게이트 그래프와 맞춰 보고, 그 맵의 문턱 레벨(가는 길에 지나는 게이트의
`required_level` 중 가장 높은 것)이 퀘스트의 `min_level` 을 넘으면 오류로 잡는다.
게이트 레벨을 올리는 순간 그 너머를 가리키던 퀘스트가 조용히 막히기 때문이다 — 데이터는
그대로고, 증상은 "받았는데 갈 수가 없다"로만 나타난다.
...
번호 없이(-1) 들어온 완료 요청은 거절한다 — 무엇을 줄지 정하지 않은 채 끝낼 수는 없다.

**대화로 끝낼 때는 선택지가 그 번호를 정한다.** 대화 응답에는 보상 번호를 실을 자리가
없으므로, 완료 선택지를 보상마다 하나씩 두고 각 선택지에 `reward_choice` 를 적는다
(`Engine/Dialog/README.md` 참고). 빠뜨리면 서버가 거절하는데 대화창에는 그대로 보이므로
"눌러도 아무 일도 일어나지 않는" 선택지가 된다 — 데이터 검증이 이것을 막는다.
