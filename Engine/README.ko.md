...
## 🗺️ 존 그래프 — 여러 navmesh 를 가로지르는 경로 탐색

navmesh 하나는 맵 하나의 지형만 안다. Detour 는 "이 맵 안에서 A→B" 는 풀지만
"마을에서 던전까지 몇 개 맵을 거쳐 가야 가장 가까운가" 는 답하지 못한다.
`ZoneGraph`(`Map/ZoneGraph.h`)가 그 위층을 맡는다.

### 그래프의 모양

정점은 맵이 아니라 **마커**다 — 게이트, 그리고 게이트가 가리키는 도착 지점(`player_spawn`).
간선은 두 종류뿐이다.

| 간선 | 잇는 것 | 비용 |
| --- | --- | --- |
| 도보 | 같은 맵 안의 마커끼리(완전 그래프) | 그 맵 navmesh 위의 실제 이동 거리 |
| 포탈 | 게이트 → 그 게이트의 `target_id` | 0 (순간이동) |

맵을 정점으로 두지 않는 이유는, 같은 맵에 게이트가 여럿이면 "어느 문으로 들어와 어느 문으로
나가느냐"에 따라 걷는 거리가 완전히 달라지기 때문이다. 마커를 정점으로 두면 그 차이가
간선 가중치로 자연스럽게 표현된다.

그래프는 `Map.json` 만으로 만들어진다. navmesh 없이도 연결 관계는 그대로 성립하므로
테스트/툴에서도 같은 경로가 나오고, navmesh 를 붙이면 도보 간선의 **비용만** 직선 거리에서
실측 거리로 바뀐다.

### 쓰는 법

```cpp
ZoneGraph::Route route = world->FindRoute(myMapId, myPos, targetMapId, targetPos, myLevel);
if (route.found)
{
    for (const ZoneGraph::Step& step : route.steps)
    {
        // step.mapId 맵에서 step.from -> step.to 까지 걷고,
        // step.gateId 게이트를 밟는다(0 이면 여기가 최종 목적지).
    }
}
```

`Route` 는 맵별 구간(`Step`)으로 쪼개져 나온다. 각 구간을 실제로 걷는 것은 그 맵의
navmesh 이고, ZoneGraph 는 **어느 게이트를 어떤 순서로 밟을지**만 정한다.

### 탐색이 지키는 규칙

- **레벨 제한**: `level` 이 1 이상이면 `required_level` 이 더 높은 게이트는 경로에서 빠진다
  (0 은 제한 없음).
- **인스턴스는 지름길이 아니다**: field 가 아닌 맵(레이드 등)은 **목적지일 때만** 지나간다.
  입장할 때마다 새 인스턴스가 만들어지므로 '반대편으로 빠져나가는 통로' 가 성립하지 않는다.
- **고립된 맵**: 들어오는 게이트가 없으면 경로가 나오지 않는다(없는 길을 지어내지 않는다).

### 도보 비용은 어디서 오나

우선순위가 셋 있다.

1. **`Map.json` 의 `gate_links`** — 같은 맵 안 두 마커 사이의 비용을 직접 적은 것.
   적혀 있으면 무조건 이긴다.
2. **navmesh 실측** — `World::Init` 이 로드된 맵의 navmesh 로 `NavMesh::PathLength` 를
   호출해 잰다. 경로가 끊겨 있으면 실패로 친다(부분 경로 길이를 쓰면 못 가는 구간이
   싼 간선으로 보여 라우팅이 통째로 어긋난다).
3. **직선 거리** — 위 둘 다 안 될 때. 인스턴스 맵처럼 기동 시 로드되지 않는 맵이 여기 해당한다.

3번으로 떨어진 간선 수는 `ZoneGraph::EstimatedEdgeCount()` 로 노출되고 기동 로그에도 남는다.
그 값이 0 이 아니면 그만큼 라우팅이 낙관적이라는 뜻이므로, 해당 구간을 `gate_links` 에
적어 두면 정확해진다.

```json
"gate_links": [
  { "from_id": 1002, "to_id": 1003, "cost": 43.81 }
]
```

방향은 대칭으로 본다(같은 통로를 반대로 걷는 것이므로). 양쪽을 다 적으면 어느 쪽이
이기는지 데이터만 보고 알 수 없어서 `validate_data.py` 가 중복으로 잡는다. 다른 맵의 마커를
가리키거나 음수 비용을 적는 것도 같은 곳에서 막힌다.

### 비용 감각 (실측: [PERFORMANCE.md](../Benchmark/PERFORMANCE.md) 15절)

| | 값 |
| --- | --- |
| 빌드 — 그래프 구성 | 간선당 ~80 ns |
| 빌드 — navmesh 실측 1회(500×500 맵) | **181 us** |
| 탐색 — 이웃 맵 | ~1 us (그래프 크기와 거의 무관) |
| 탐색 — 세계 반대편(정점 3,968) | 365 us |
| 탐색 — 없는 맵 | 0.012 us (시작 전에 걸러진다) |

두 가지만 기억하면 된다.

- **빌드 비용은 navmesh 실측이 지배한다.** 그래프 구성보다 2,000배 비싸다. 큰 맵에 게이트가
  10개면 그 맵 하나로 ~8 ms 다 — `gate_links` 에 적어 두면 그 간선은 Detour 를 건너뛴다.
- **흔한 요청(이웃 맵)은 싸지만 먼 거리는 아니다.** 다익스트라가 목적지를 확정하면 멈추므로
  가까운 곳은 그래프가 커져도 ~1 us 다. 반면 세계를 가로지르는 조회는 틱마다 부를 것이 못 된다.
  연결 여부만 필요하면 `AreConnected` 가 7.5배 싸다.

### 검증

정확성은 격자 세계에서 확인한다(`UnitTest/SyntheticMaps.h`). W×H 격자에서는 두 맵 사이의
최소 전환 횟수가 맨해튼 거리와 같으므로, 탐색 결과를 손으로 검산할 수 있다 — 실제 `Map.json`
은 두 맵을 잇는 길이 하나씩뿐이라 "최단을 고르는지" 를 확인할 수 없다.
벤치마크(`Benchmark/ZoneGraphBenchmark.cpp`)도 같은 위상을 써서, 측정 대상과 검증 대상이
갈라지지 않게 했다.
