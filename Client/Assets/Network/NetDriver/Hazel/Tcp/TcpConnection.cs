...
namespace Hazel.Tcp
{
    ...
    public sealed class TcpConnection : NetworkConnection
    {
            ...
            lock(socketLock)
            {
                ...
                try
                {
                    IAsyncResult result = socket.BeginConnect(RemoteEndPoint, new AsyncCallback((IAsyncResult ar) => 
                    {
                        ...
                        {
                            core.LogHelper.LogError($"An exception occured while EndConnect {e}");
                            return;
                        }

                        core.LogHelper.LogInfo($"Socket connected to {tcpConnection.socket.RemoteEndPoint.ToString()}");
                        ...
                        {
                            core.LogHelper.LogError($"An exception occured while initiating the first receive operation. {e}");
                            return;
                        }
        ...
        ///     </para>
        /// </remarks>
        public override void SendBytes(byte[] bytes, int lengthBits, SendOption sendOption = SendOption.FragmentedReliable)
        {
            //Get bytes for length
            byte[] fullBytes = AppendLengthHeader(bytes, lengthBits);
        ...
        static byte[] AppendLengthHeader(byte[] bytes, int lengthBits)
        {
            byte[] fullBytes;
            ...
            if (lengthBits != 0)
            {
                fullBytes = new byte[bytes.Length + 6];

                uint length = (uint)bytes.Length + 2;

                //Append length
                fullBytes[0] = (byte)(((uint)length >> 24) & 0xFF);
                fullBytes[1] = (byte)(((uint)length >> 16) & 0xFF);
                fullBytes[2] = (byte)(((uint)length >> 8) & 0xFF);
                fullBytes[3] = (byte)(uint)length;


                fullBytes[4] = (byte)(((uint)lengthBits >> 8) & 0xFF);
                fullBytes[5] = (byte)(uint)lengthBits;

                //Add rest of bytes
                Buffer.BlockCopy(bytes, 0, fullBytes, 6, bytes.Length);
            }
            else
            {
                fullBytes = new byte[bytes.Length + 4];
