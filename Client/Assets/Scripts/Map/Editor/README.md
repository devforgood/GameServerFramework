...
- **씬은 Map.json 으로 되살릴 수 없습니다.** JSON 이 담는 것은 마커(게이트·스폰·오브젝트)뿐이고,
  지형(바닥·장애물·경사로·경계 벽)은 씬에만 있습니다. 씬 파일을 잃으면 2단계부터 다시 만들어야 합니다.
- OBJ 추출은 씬 전체를 대상으로 하되 **NavMeshVisualizer 가 만든 폴리곤 메시는 뺍니다.**
  그것은 지난번에 구운 navmesh 를 눈으로 보라고 만든 것이라, 같이 구우면 지난 결과가 지형인 척
  다시 들어갑니다(씬 하나에 100개씩 들어 있어 정점 예산도 그만큼 먹습니다).
...
### 게이트는 씬에 없어도 된다 (런타임 정리)

게이트는 클라에서 **트리거 콜라이더로만** 존재합니다(`Gate.OnTriggerEnter` → `EnterGate`).
예전에는 씬 파일에 GameObject 가 없으면 그 게이트를 지나갈 수 없어서, 데이터에 게이트를
추가할 때마다 유니티에서 씬을 열어 맞춰 주는 손일이 따라붙었습니다. 그리고 실제로 어긋났습니다 —
게이트 1007/1008 은 씬에 아예 없었고, 1001/1002 는 낡은 `targetId` 를 들고 있었습니다.

지금은 맵 씬이 준비될 때마다
[`MapSceneGates.Reconcile`](../MapSceneGates.cs) 이 Map.json 기준으로 정리합니다.

| 상황 | 하는 일 |
| --- | --- |
| 데이터에 있고 씬에 없음 | 트리거를 만든다(kinematic Rigidbody + BoxCollider) |
| 둘 다 있음 | 씬 오브젝트를 그대로 두고 `type`/`targetId`/`requiredLevel`/위치만 데이터로 맞춘다 |
| 씬에만 있음 | 지운다 — 밟아도 서버가 모르는 id 라 거절한다 |

**위치까지 데이터를 따르는 이유**: 서버는 도착한 플레이어를 게이트의 *데이터 좌표*에 세웁니다.
씬 쪽 위치가 다르면 플레이어는 트리거가 없는 자리에 도착해, 게이트가 고장 난 것처럼 보입니다.

그래서 씬에 게이트를 배치하는 것은 이제 **선택**입니다(외형을 주고 싶을 때 `Gate.prefab` 을 씁니다).
게이트를 옮기고 싶으면 씬에서 옮기고 Scan 으로 JSON 에 기록하면 됩니다 — 그러면 다음 실행에서도
그 자리에 섭니다. 씬에만 옮겨 두고 저장하지 않으면 런타임에 데이터 위치로 되돌아갑니다.

> **트리거에는 Rigidbody 가 필요합니다.** 유니티는 두 콜라이더 중 한쪽에 Rigidbody 가 있어야
> 트리거를 발생시키는데, 플레이어(`Resources/Character2`)는 콜라이더뿐입니다. 그래서 게이트가
> kinematic Rigidbody 를 들고 있어야 합니다 — `Gate.prefab` 도, 런타임 생성도, `Build Scene ← JSON`
> 도 그렇게 만듭니다. 직접 만든 게이트가 반응하지 않으면 여기부터 보세요.

### 씬이 없는 맵

맵의 `scene` 이 실제 씬 에셋이 아니거나 Build Settings 에 없으면 저장할 때 경고가 뜹니다.
서버는 Map.json 만 읽으므로 그런 맵도 멀쩡히 띄우지만, **클라는 들어갈 수 없습니다**
(`MapTransition.LoadMapScene` 이 경고만 남기고 현재 씬에 머뭅니다).
데이터만 봐서는 드러나지 않는 종류의 구멍이라 툴이 대신 봅니다.
