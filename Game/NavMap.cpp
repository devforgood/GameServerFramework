...
int NavMap::addAgent(const float* p, float speed = 3.5f)
{
	dtCrowd* crowd = m_crowd;
...
void NavMap::removeAgent(const int idx)
{
...
void NavMap::setMoveTarget(const float* p, bool adjust, const int agent_idx)
{
...
bool NavMap::raycast(int agent_idx, const float* endPos, float * hitPoint)
{
...
bool NavMap::patrol(int agent_idx, const float* startPos, dtPolyRef startRef)
{
	float epos[3];
	dtPolyRef endRef;
...
const float* NavMap::getPos(const int agent_idx)
{
	return m_crowd->getAgent(agent_idx)->npos;
}

const dtCrowdAgent* NavMap::getAgent(const int agent_idx)
{
	return m_crowd->getAgent(agent_idx);
}

// 에이전트를 지정된 위치로 순간이동합니다.
bool NavMap::teleportAgent(int agent_idx, const float* pos)
{
	dtCrowd* crowd = m_crowd;
	if (crowd)
	{
