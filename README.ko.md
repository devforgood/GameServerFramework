...
이 저장소의 구현, 리팩터링, 성능 분석, 문서 작업 일부는 AI 코딩 어시스턴트(주로 Claude Code)의 도움을 받아 작성되었습니다. AI가 만든 결과물을 그대로 커밋하지는 않으며, 사람이 검토하고 빌드·테스트를 거친 뒤 반영합니다. 어시스턴트에게 적용하는 코딩 규칙은 [AGENTS.md](AGENTS.md)에 정리되어 있습니다.
...
| 명령 | 하는 일 |
| --- | --- |
| `spawn <monsterId> [count] [radius]` | 내 주변에 몬스터를 세웁니다. 마커 스폰이 아니라서 잡으면 다시 서지 않습니다. |
| `killall [radius]` | 맵(반경을 주면 그 안)의 몬스터를 전부 처치합니다. 킬 크레딧이 내 캐릭터로 남아 경험치/퀘스트가 정상 처치와 같게 진행됩니다. |
| `heal` / `hp <value>` / `die` | 체력을 최대치로 채우거나, 지정한 값으로 만들거나, 캐릭터를 죽입니다(부활 흐름 확인용). |
| `god [on\|off]` | 무적을 켜고 끕니다(인자가 없으면 토글). 맵을 옮기면 캐릭터가 새로 만들어져 풀립니다. |
| `level <level>` / `exp <amount>` | 레벨을 직접 세우거나 경험치를 지급합니다. |
...
명령표는 [CheatCommands.cpp](Engine/Cheat/CheatCommands.cpp) 의 `kCommands` 한 곳에 있습니다. 표에 한 줄 넣고 함수를 하나 쓰면 `help` 와 클라이언트 자동완성에 함께 반영됩니다.

입력창에서 `/` 로 시작하면 자동완성이 뜹니다. 목록은 서버에서 받고(`syncnet::CheatList`), 인자 id 는 클라이언트가 같은 GameData 에서 만듭니다.

| 키 | 하는 일 |
| --- | --- |
| `Tab` | 후보가 하나면 완성, 여럿이면 공통 접두사까지 채웁니다. |
| `↑` `↓` | 후보를 고릅니다. 후보가 없으면 이전에 보낸 줄을 되불러옵니다. |
| `Enter` | 고른 후보가 있으면 그것으로 채우고, 없으면 지금 줄을 보냅니다. |
| `Esc` | 후보 목록만 닫습니다. 목록이 없으면 창을 닫습니다. |

치트는 `server_config.json` 의 `network.allow_debug_commands` 가 `true` 일 때만 실행됩니다(운영 기본값은 `false`). 꺼져 있으면 조용히 무시하지 않고 꺼져 있다고 되돌려 주며, 자동완성 목록도 비어서 내려갑니다(운영 서버가 명령 이름을 흘리지 않습니다).
...
백엔드는 프로세스 전역이 아니라 **몬스터마다 다를 수 있습니다.** `MonsterBTRunner` 가 원래 개체별이라 전역이었던 것은 "어느 백엔드를 고를까" 하나뿐이었고, 그 선택을 `Monster::ResolveBTBackend` 로 옮겼습니다. 한 맵에서 섞여도 서로를 방해하지 않습니다 — `MonsterAISystem` 은 등록된 컴포넌트만 훑으므로 트리 백엔드 몬스터는 그 배열에 없고, ECS 몬스터의 `Tick` 은 무동작입니다. 지금 들어 있는 규칙은 하나입니다: `Monster::debugBossOnBTCpp_` 를 켜면 보스만 `BTCpp` 로 스폰되어 [BT 디버그 뷰어](Engine/AI/BTDebugManager.h)로 판단 과정을 들여다볼 수 있고, 나머지는 ECS 에 남습니다. 기본값은 꺼짐입니다 — BTCpp 는 스폰마다 `Monster.xml` 을 파싱하므로 상시 켤 값이 아닙니다.
