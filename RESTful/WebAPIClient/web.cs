...
namespace WebAPIClient
{
    public class Web
    {
        ...
        public static async Task<bool> writePurchaseLog(string playerId, RequestWritePurchaseLog body)
        {
            // Call asynchronous network methods in a try/catch block to handle exceptions.
            try
            {
                Log.Information($"writePurchaseLog {playerId}, {body}");

                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("KakaoAK", Authorization);

                HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Post, $"{ServerURL}/log/writePurchaseLog");
                msg.Content = new StringContent(JsonConvert.SerializeObject(body), Encoding.UTF8, "application/json");
                msg.Headers.Add("appId", appId);
                msg.Headers.Add("appSecret", appSecret);
                msg.Headers.Add("playerId", playerId);
        ...
        public static async Task<(bool, ResponseRemovedPlayers)> getRemovedPlayersWithBucket(long dayTimeSlot, int bucketFrom = 0, int size = 200)
        {
            try
            {
                var body = new RquestRemovedPlayers();
                body.dayTimeSlot = dayTimeSlot;
                body.bucketFrom = bucketFrom;
                body.size = size;

                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("KakaoAK", Authorization);

                HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Post, $"{ServerURL}/player/getRemovedPlayersWithBucket");
                msg.Content = new StringContent(JsonConvert.SerializeObject(body), Encoding.UTF8, "application/json");
                msg.Headers.Add("appId", appId);
                msg.Headers.Add("appSecret", appSecret);
                ...
                var responseData = JsonConvert.DeserializeObject<ResponseRemovedPlayers>(responseString);
                Log.Information($"validate ok :{responseString}");
